...
const FilterCards = ({ isFilterOpen, setIsFilterOpen }) => {
  ...
  useEffect(() => {
    const { Location, Industry, Salary, Skills } = selectedValues;
    const queryArray = [];

    if (Location.length > 0) {
      queryArray.push(...Location); // push all selected locations
    }

    if (Industry.length > 0) {
      queryArray.push(...Industry); // push all selected industries
    }

    if (Salary !== "") {
      const range = Salary.replace("LPA", "").split("-"); // e.g., "51-100"
      const start = parseInt(range[0]);
      const end = parseInt(range[1]);

      for (let i = start; i <= end; i++) {
        queryArray.push(i.toString()); // e.g., "51", "52", ..., "100"
      }
    }

    if (Skills !== "") {
      queryArray.push(Skills); // push selected skill
    }

    dispatch(setSearchedQuery(queryArray.join(" ")));
